...
import { formatFileSize } from "@/features/resizer/utils/resize";
...
export function ResizeResults() {
  ...
  const cropAndDownload = useResizerStore((s) => s.cropAndDownload);
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="rounded-xl bg-background p-3">
          ...
          <div className="mt-1.5 grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-text-muted">
            <span>Input: {Math.round(cropWidth)} × {Math.round(cropHeight)}px</span>
          ...
          >
            {isProcessing ? (
              ...
              <>
                ...
                {hasResult ? "Download Again" : "Crop & Download"}
              </>
            )}
          </Button>
          <Button
