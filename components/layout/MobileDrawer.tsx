...
import { ArrowRight, ChevronDown, House, Info, Mail, Newspaper, X } from "lucide-react";
        ...
        <div className="md:hidden">
          ...
          >
            ...
            <nav className="flex-1 overflow-y-auto px-3 py-4" aria-label="Mobile navigation links">
              ...
              <NavGroup title="More" count={pageLinks.length}>
                {pageLinks.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    onClick={onClose}
                    className={cn(
                      "flex items-center gap-2.5 rounded-lg border border-border/70 bg-background/60 px-3 py-2.5 text-sm font-medium transition-colors",
                      pathname === link.href
                        ? "border-primary/40 bg-primary-light/60 text-primary"
                        : "text-text-secondary hover:border-primary/30 hover:bg-primary-light/50 hover:text-primary"
                    )}
