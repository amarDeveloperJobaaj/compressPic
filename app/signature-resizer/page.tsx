...
import { SignatureControls } from "@/features/signature-resizer/components/SignatureControls";
import { SignaturePreview } from "@/features/signature-resizer/components/SignaturePreview";
import { PageTransition } from "@/components/shared/PageTransition";

const UploadZone = dynamic(
  () =>
    import("@/features/signature-resizer/components/UploadZone").then((m) => ({
...
export default function SignatureResizerPage() {
  const originalFile = useSignatureResizerStore((s) => s.originalFile);
  const error = useSignatureResizerStore((s) => s.error);
  const reset = useSignatureResizerStore((s) => s.reset);
      ...
      <div className="container-page py-10 sm:py-16">
        ...
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h1 className="mt-4 text-3xl font-bold tracking-tight text-text-primary sm:text-4xl">
            Resize Your Signature Online
          </h1>
          <p className="mt-3 text-lg text-text-secondary">
            Fit your signature to any size — 300×100 to 1000×300 px — and squeeze
            it under common file limits like 20 or 50 KB. All in your browser.
          </p>
        </div>
        ...
        {!originalFile && (
          <div className="mx-auto mt-8 max-w-xl text-center">
            <p className="text-sm text-text-muted">
              No uploads. No servers. 100% private browser-based signature resizing.
            </p>
          </div>
        )}
      </div>
